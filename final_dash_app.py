...
format_fig = df['UNIT'].unique()
country_ind = df['GEO'].unique()
available_ind = df['NA_ITEM'].unique()
...
app.layout = html.Div([
    html.Div([
        ...
        html.Div([
            ...
            dcc.RadioItems(
                id='value-format',
                options=[{'label': i, 'value': i} for i in format_fig],
    ...
    html.Div([
        ...
        html.Div([
            dcc.Dropdown(
                id='country-select',
                options=[{'label': i, 'value': i} for i in country_ind],
                value='Iceland'
            )
            ],style={'width': '48%', 'float': 'right', 'display': 'inline-block'})
    ]),
